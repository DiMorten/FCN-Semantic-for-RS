...
class Dataset(NetObject):
    ...
    def patches_extract(self, image):
        ...
        patches['label'] = self.view_as_windows_multichannel(
            image['label'], (self.patch_len, self.patch_len, 1), step=self.patch_step)
...
class NetModel(NetObject):
    def __init__(self, batch_size=1, batch_size_test=10, epochs=2, eval_mode='metrics', *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.debug >= 1:
    ...
    def train_loop(self, data):
        ...
        for epoch in range(self.epochs):
            ...
            print("Train loss={}, Test loss={}".format(self.metrics['test']['loss'],self.metrics['test']['loss']))
...
if __name__ == '__main__':
    ...
    model = NetModel(epochs=args.epochs, patch_len=args.patch_len,
                     patch_step=args.patch_step, eval_mode=args.eval_mode)
    model.build()
